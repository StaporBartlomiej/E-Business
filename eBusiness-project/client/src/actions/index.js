...
export const getProducts = () => async dispatch => {
    try {
        ...
        products.map(product => {
            let {price: pricee, name} = product;
            product.cost = pricee;
            product.title = name;
            pricee = parseInt(pricee.substring(0, pricee.length - 1), 10);
            pricee > 1000 ? product.freeDelivery = true : product.freeDelivery = false;
...
export const getProduct = id => async dispatch => {
    try {
        ...
        rightProduct.cost = product[0].price;
